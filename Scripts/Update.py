...
OUTPUT_M3U = os.path.join(ROOT_DIR, "Data", "OnePiece_Egghead.m3u")

# File that remembers which episodes we've already seen (so we don't re-download everything)
KNOWN_IDS_FILE = os.path.join(ROOT_DIR, "Scripts", "known_ids.json")
...
def build_m3u_from_files(files, base_url):
    ...
    for f in files:
        ...
        # If Pixeldrain changes formats again, just update this line:
